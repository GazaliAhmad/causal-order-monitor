...
CREATE INDEX idx_ingress_events_expires_at
  ON ingress_events(expires_at_ms);

CREATE INDEX idx_ingress_events_replay_state
  ON ingress_events(replay_state);

CREATE INDEX idx_ingress_events_replay_ingest_time
  ON ingress_events(replay_state, monitor_ingest_at_ms);

CREATE INDEX idx_ingress_events_replay_expiry
  ON ingress_events(replay_state, expires_at_ms);

CREATE INDEX idx_ingress_events_source_path
...
CREATE INDEX idx_component_health_log_component_time
